...
export function useSelectionAssistant() {
  ...
  const actionItems = selectionStore.actionItems.filter((item) => item.id !== 'translate')

  useEffect(() => {
    if (selectionStore.triggerMode === 'shortcut') {
      dispatch(setTriggerMode('selected'))
      void window.api.selection.setTriggerMode('selected')
    }
  }, [dispatch, selectionStore.triggerMode])
  ...
  return {
    ...
    setActionItems: (items: ActionItem[]) => {
      dispatch(setActionItems(items.filter((item) => item.id !== 'translate')))
    }
  }
}
