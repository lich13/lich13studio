...
const migrateConfig = {
  ...
  '136': (state: RootState) => {
    try {
      state.settings.sidebarIcons.visible = [...new Set(state.settings.sidebarIcons.visible)].filter(
        (icon): icon is SidebarIcon => DEFAULT_SIDEBAR_ICONS.includes(icon as SidebarIcon)
      )
      state.settings.sidebarIcons.disabled = [...new Set(state.settings.sidebarIcons.disabled)].filter(
        (icon): icon is SidebarIcon => DEFAULT_SIDEBAR_ICONS.includes(icon as SidebarIcon)
      )
      return state
    } catch (error) {
  ...
  '208': (state: RootState) => {
    try {
      ...
      if (state.selectionStore?.triggerMode === 'shortcut') {
        state.selectionStore.triggerMode = 'selected'
      }

      if (state.settings?.apiServer) {
        state.settings.apiServer.enabled = false
      }
