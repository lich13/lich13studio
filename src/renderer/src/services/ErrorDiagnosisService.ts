...
async function buildModelsToTry(context?: DiagnosisContext): Promise<Model[]> {
  ...
  // User's default model as primary diagnosis model.
  ...
  for (const provider of [...providers.filter((provider) => provider.enabled), ...providers.filter((provider) => !provider.enabled)]) {
    ...
    if (model.id === context?.modelId) {
      continue
    }
    if (!models.some((candidate) => candidate.id === model.id && candidate.provider === model.provider)) {
      models.push(model)
    }
  }
...
function buildContextHint(errorInfo: Record<string, unknown>, context?: DiagnosisContext): string {
  ...
  if (
    ...
    return `## Context\nThe user is calling ${provider} API and got an authentication error. lich13studio lets users configure API keys per provider in provider settings.\n`
  }
  ...
  // Generic
  return `## Context\nlich13studio is an AI chat app connecting to LLM providers (OpenAI, Anthropic, Gemini, and custom endpoints) with API keys. Error occurred during ${source || 'chat'}.\n`
}
...
export async function diagnoseError(
  ...
  const prompt = `You are an error diagnosis assistant for lich13studio, an AI chat desktop app.
...
export async function classifyErrorByAI(error: SerializedError, language: string): Promise<string> {
  const prompt = `You are an error diagnosis assistant for lich13studio. Summarize this error in one sentence (max 30 words) in ${language}. Return ONLY the summary text, no JSON, no markdown, no quotes.`
