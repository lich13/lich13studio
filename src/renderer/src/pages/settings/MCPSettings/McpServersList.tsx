      ...
      <Sortable
        ...
        renderItem={(server) => (
          <McpServerCard
            ...
            version={serverVersions[(server as MCPServer).id]}
            isLoading={loadingServerIds.has((server as MCPServer).id)}
            onToggle={async (active) => await handleToggleActive(server as MCPServer, active)}
            onDelete={() => onDeleteMcpServer(server as MCPServer)}
            onEdit={() => navigate(`/settings/mcp/settings/${encodeURIComponent((server as MCPServer).id)}`)}
