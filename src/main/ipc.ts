...
export async function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  ipcMain.handle(IpcChannel.Backup_WriteWebdavTextFile, backupManager.writeWebdavTextFile.bind(backupManager))
  ...
  ipcMain.handle(IpcChannel.ApiServer_GetStatus, removedFeatureHandler('API server'))
  ipcMain.handle(IpcChannel.ApiServer_GetConfig, removedFeatureHandler('API server'))
