...
class BackupManager {
  ...
  async backupToWebdav(_: Electron.IpcMainInvokeEvent, webdavConfig: WebDavConfig) {
    const filename = webdavConfig.fileName || 'lich13studio.backup.zip'
  ...
  private async restoreDirect(): Promise<void> {
    ...
    try {
      ...
      // Accept backups produced by both the upstream app and lich13studio.
      if (!['Cherry Studio', 'lich13studio'].includes(metadata.appName)) {
        throw new Error('This backup file is not from a supported app and cannot be restored')
      }
